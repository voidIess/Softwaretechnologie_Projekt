...
Dieser Unterabschnitt beschreibt die wichtigsten Anwendungsfälle des Systems. Wichtig sind solche Anwendungsfälle, die für den Auftraggeber, den Nutzer den größten Nutzen bringen.
...
=== 7.1 Muss-Kriterien
Anforderungen, die das Programm auf alle Fälle erfüllen muss.
...
== 9 GUI Prototyp
In diesem Kapitel wurde ein Entwurf der Navigationsmöglichkeiten und Dialoge des Systems erstellt. Zusätzlich gibt es auch einen ersten grafiscgen Prototyp, der zeigen soll, wie das System visuell umgesetzt werden soll.
...
|Dienstplan | Der Dienstplan ist eine Übersicht für die Mitarbeiter, in der sie erkennen können, wer wann als was arbeiten muss.
|Trainingsplan| Trainingsplan ist eine Übersicht über die anstehenden Trainings des Kunden des Fitnessstudios.
|Trainingstermin| Trainingstermin beschreibt einen Termin in dem Trainingsplan.
|Guthabenkonto|Das Guthabenkonto ist ein Konto des Kunden des Fitnessstudios. Auf dieses Konto kann er Geld einzahlen, jedoch nicht wieder abheben. Über dieses Konto kann er z.B. Käufe an der Theke bezahlen. Auch Kundenkonto genannt.
|Artikel|Ein Artikel beschreibt Gegenstände, welche im Fitnessstudio erworben werden können. Dazu zählen unter anderem Getränke und Nahrungsergänzungsmittel, allerdings auch Hantel und andere Trainingsgeräte.
|Inventar| Das Inventar beschreibt die Gesamtheit aller Artikel des Fitnessstudios.
|Nachbestellliste|Die Nachbestellliste enthält alle Artikel, welche nachgekauft werden müssen, da sie entweder abgelaufen sind, oder zu wenig Exemplare vorhanden sind. Dabei handelt es sich jedoch nicht um eine wirklich Liste. Die Liste wird nur im Hintergrund verwendet um im Inventar die entsprechenden Artikel zu markieren.
|Einchecken/Auschecken| Wenn ein Kunde das Fitnessstudio betritt, wird er im System angemeldet (eingecheckt). So kann der Chef des Fitnessstudios Statistiken über Anwesenheitszeit und Sonstiges sehen. Verlässt er das Fitnessstudio, wird dies ebenfalls im System vermerkt (ausgecheckt).
|Code (ReferralCode)| Jeder Kunde bekommt so einen Code. Diesen Code kann er seinen Freunden und Bekannten geben. Geben sie diesen Code beim Registrieren im Fitnessstudio an, so erhält der Besitzer des Codes eine Prämie.
|===
